...
# $PROXY_DIR ${file} "http://localhost:${tiny_port}/${file}" "http://localhost:${proxy_port}"
function download_proxy {
    cd $1
    curl --max-time ${TIMEOUT} --silent --proxy $4 --output $2 $3
    # echo "curl --max-time ${TIMEOUT} --silent --proxy $4 --output $2 $3"
...
function download_noproxy {
    ...
    # echo "curl --max-time ${TIMEOUT} --silent --output $2 $3"
    (( $? == 28 )) && echo "Error: Fetch timed out after ${TIMEOUT} seconds"
    cd $HOME_DIR
}
