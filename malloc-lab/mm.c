...
#define SET_PREV(p, prev) (*(unsigned int *)(p) = (unsigned int)(prev))
#define GET_NEXT(p) (*((unsigned int *)(p) + 1))
#define SET_NEXT(p, val) (*((unsigned int *)(p) + 1) = (unsigned int)(val))
...
static void *first_fit(size_t asize)
{
    for (void *bp = free_list_head; bp != 0; bp = (void *)GET_NEXT(bp))
    {
